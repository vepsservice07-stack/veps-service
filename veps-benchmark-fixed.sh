...
for i in $(seq 1 100); do
    ...
    if command -v jq &> /dev/null && [ -n "$BODY" ]; then
        TOTAL=$(echo "$BODY" | jq -r '.data.performance_breakdown.total_ms // 0' 2>/dev/null)
        VEPS_INTERNAL=$(echo "$BODY" | jq -r '.data.performance_breakdown.veps_internal_ms // 0' 2>/dev/null)
        PARSING=$(echo "$BODY" | jq -r '.data.performance_breakdown.parsing_ms // 0' 2>/dev/null)
        NORMALIZATION=$(echo "$BODY" | jq -r '.data.performance_breakdown.normalization_ms // 0' 2>/dev/null)
        ROUTING=$(echo "$BODY" | jq -r '.data.performance_breakdown.routing_ms // 0' 2>/dev/null)
        SUCCESS=$(echo "$BODY" | jq -r '.success // false' 2>/dev/null)
    else
        # Fallback if jq fails or body is empty
        TOTAL=0
        VEPS_INTERNAL=0
        PARSING=0
        NORMALIZATION=0
...
VEPS_P50=$(awk -F',' 'NR > 1 && $4 ~ /^[0-9.]+$/ && $4 > 0 {v[NR]=$4;c++} END {n=asort(v);if(n>0)print v[int(n*0.50)];else print 0}' "$CSV_FILE")
VEPS_P99=$(awk -F',' 'NR > 1 && $4 ~ /^[0-9.]+$/ && $4 > 0 {v[NR]=$4;c++} END {n=asort(v);if(n>0)print v[int(n*0.99)];else print 0}' "$CSV_FILE")
TOTAL_P50=$(awk -F',' 'NR > 1 && $3 ~ /^[0-9.]+$/ && $3 > 0 {v[NR]=$3;c++} END {n=asort(v);if(n>0)print v[int(n*0.50)];else print 0}' "$CSV_FILE")
TOTAL_P99=$(awk -F',' 'NR > 1 && $3 ~ /^[0-9.]+$/ && $3 > 0 {v[NR]=$3;c++} END {n=asort(v);if(n>0)print v[int(n*0.99)];else print 0}' "$CSV_FILE")

# Check if we got valid data
if [ "$VEPS_P50" == "0" ] || [ -z "$VEPS_P50" ]; then
